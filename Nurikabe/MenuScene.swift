...
final class MenuScene: SKScene {
    ...
    private func startGame() {
        // Transition to game scene with sleek zoom+fade
        guard let gameScene = GameScene(fileNamed: "GameScene") else {
            assertionFailure("GameScene.sks not found")
            return
        }
        gameScene.scaleMode = .aspectFill
        let door = SKTransition.doorsOpenVertical(withDuration: 0.55)
        view?.presentScene(gameScene, transition: door)
    }
